...
def list_m(a, r, Htot, iteration = 25):
    ...
    mValues = np.hstack((AUX1, AUX2, BesselZeros[3:iteration+1]))
    mValues = np.sort(mValues)
...
def list_quad_m (a, r, Htot, iteration = 25):
...
print(f'Il y a {len(lm)-1} intervalles')

l=list_quad_m(a, r,Htot,iteration)

print (f"Il y a {len(l)} points d'intégration")
'''
...
print (f'le min est {l[0]} et le max est {l[-1]}')

val_r= (l[-1]**3 - l[0]**3)/3

print (f'la valeur réelle est {val_r}')

t_puiss = t[:,0]**2
t_pond = t_puiss * t[:,1]

val_quad=np.sum(t_pond)
print (f'la valeur intégrée est {val_quad}')

ecart=(val_r-val_quad)/val_r*100

print (f"l''écart est de {ecart} %")
